-- Vehicle Maintenance Management System Database Schema (BIGINT版本)

-- 先删除所有视图和触发器
DROP VIEW IF EXISTS FinancialSummary;
DROP VIEW IF EXISTS MaterialConsumptionReport;
DROP VIEW IF EXISTS StaffPerformance;
DROP VIEW IF EXISTS UserVehicleHistory;

DROP TRIGGER IF EXISTS update_material_stock;
DROP TRIGGER IF EXISTS update_labor_costs;
DROP TRIGGER IF EXISTS update_workorder_costs;

-- 然后删除所有表（注意顺序，先删除引用表，后删除被引用表）
